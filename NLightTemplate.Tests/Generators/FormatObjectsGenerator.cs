...
namespace NLightTemplate.Tests.Generators
{
    ...
    {
        private readonly NumericTypes _numericTypes = new();
        private readonly DateTypes _dateTypes = new();
            ...
            [
                .. typeof(NumericTypes).GetTypeInfo().DeclaredProperties.Where(p => p.GetMethod?.IsPublic ?? false)
                    .SelectMany(prop =>
                        ...
                        .Select(fmt => new object[] { _numericTypes, $"{{{prop.Name}:{fmt}}}", string.Format($"{{0:{fmt}}}", prop.GetValue(_numericTypes)) })
                    ),
                .. typeof(NumericTypes).GetTypeInfo().DeclaredProperties.Where(p => p.GetMethod?.IsPublic ?? false)
                        .SelectMany(prop =>
                            prop.GetCustomAttribute<FormatsAttribute>().Formats
                            .Select(fmt => new object[] { _numericTypes, $"{{{prop.Name},10:{fmt}}}", string.Format($"{{0,10:{fmt}}}", prop.GetValue(_numericTypes)) })
                        ),
                .. typeof(DateTypes).GetTypeInfo().DeclaredProperties.Where(p => p.GetMethod?.IsPublic ?? false)
                        .SelectMany(prop =>
                            prop.GetCustomAttribute<FormatsAttribute>().Formats
                            .Select(fmt => new object[] { _dateTypes, $"{{{prop.Name}:{fmt}}}", string.Format($"{{0:{fmt}}}", prop.GetValue(_dateTypes)) })
                        ),
            ];
        ...
        {
            [Formats(["C3", "D4", "e1", "E2", "F1", "G", "N1", "P0", "X4", "0000.0000"])]
            ...
            [Formats(["G", "C", "D8", "E4", "e3", "F", "N", "P", "X", "0000.0000"])]
            ...
            [Formats(["G", "C", "E04", "F", "N", "P", "0,0.000", "#,#.00#;(#,#.00#)"])]
            public decimal NumericTypeDecimal { get; set; } = new Random().Next() * 1.1M;
            [Formats(["C", "E", "e", "F", "G", "N", "P", "R", "#,000.000", "0.###E-000", "000,000,000,000.00###"])]
        ...
        {
            [Formats([ "d", "D", "f", "F", "g", "G", "m", "o", "R", "s", "t", "T", "u", "U", "y",
